    ...
    return (
        <>
            ...
            <div className="text-white p-6 rounded-lg shadow-lg max-w-screen-lg mx-auto">
                ...
                {/* Toggle buttons to switch between Card View and Table View */}
                
                
                <button onClick={() => setEditMode(!editMode)}
                    className="bg-blue-600 text-white px-4 py-2 rounded-md mb-6 mr-4"
                >
                    {editMode ? "Disable Edit Mode" : "Enable Edit Mode"}
                </button>
                <button
                    onClick={handleSaveChanges}
                    className="bg-green-600 text-white px-4 py-2 rounded-md mb-6 mr-4"
                >
                    Save Changes
                </button>
                <button
                        ...
                        <table className="min-w-full table-auto text-left border-collapse border border-gray-700 table-fixed">
                            <thead>
                                <tr className="bg-gray-800">
                                    <th className="px-4 py-2 text-xs text-gray-300 border border-gray-500" >Catch ID</th>
                                    <th className="px-4 py-2 text-xs text-gray-300 border border-gray-500">Date</th>
                                    <th className="px-4 py-2 text-xs text-gray-300 border border-gray-500">Latitude</th>
                                    <th className="px-4 py-2 text-xs text-gray-300 border border-gray-500">Longitude</th>
                                    <th className="px-4 py-2 text-xs text-gray-300 border border-gray-500">Depth</th>
                                    <th className="px-4 py-2 text-xs text-gray-300 border border-gray-500">Total Weight</th>
                                    <th className="px-4 py-2 text-xs text-gray-300 border border-gray-500">Species</th>
                                    {editMode && <th className="px-4 py-2 text-xs text-gray-300 border border-gray-500 w-20">Actions</th>}
                                    </tr>
                            </thead>
                            <tbody>
                                {catchData.map((data) =>
                                    data.catches.map((catchItem) => (
                                        <tr key={catchItem._id} className="border-b border border-gray-500">
                                            <td className="px-4 py-2 text-sm text-gray-300 border border-gray-500">{catchItem._id}</td>
                                            <td className="px-4 py-2 text-sm text-gray-300 border border-gray-500">
                                                <input
                                                    type="date"
                                                    value={editMode ? new Date(catchItem.date).toISOString().split("T")[0] : new Date(catchItem.date).toISOString().split("T")[0]}
                                            ...
                                            <td className="px-4 py-2 text-sm text-gray-300 border-b border border-gray-500">
                                                <input
                                                    type="number"
                                                    value={editMode ? catchItem.latitude : catchItem.latitude}
                                                    onChange={(e) =>
                                            ...
                                            <td className="px-4 py-2 text-sm text-gray-300 border-b border border-gray-600">
                                                <input
                                                    type="number"
                                                    value={editMode ? catchItem.longitude : catchItem.longitude}
                                                    onChange={(e) =>
                                            ...
                                            <td className="px-4 py-2 text-sm text-gray-300 border-b border border-gray-500">
                                                <input
                                                    type="number"
                                                    value={editMode ? catchItem.depth || "" : catchItem.depth || ""}
                                            ...
                                            <td className="px-4 py-2 text-sm text-gray-300 border-b border border-gray-500">
                                                <input
                                                    type="number"
                                                    value={editMode ? catchItem.total_weight : catchItem.total_weight}
                                                    onChange={(e) =>
                                            ...
                                            <div className="flex gap-8 flex-wrap">
                                                {catchItem.species.map((species) => (
                                                    <div key={species._id} className="flex justify-between items-center text-sm gap-2">
                                                        ...
                                                        <input
                                                            type="number"
                                                            value={editMode ? species.catch_weight : species.catch_weight}
                                                            onChange={(e) =>
